...
- i.e. the "surface area" for statically linking binaries to static/dynamic binaries that link to kernel may have unexpected "incompatible facets"
  ...
  + a more clear example: you built a custom kernel, but the kernel compiler
    ...
    + obviously, this shouldn't really happen (the c/c++ headers spec the API and future builds/ELFS need to link against a standard without too much compile-time or run-time config). it is mostly handled in c/c++ specs and compiler spec/implementation, but to prevent it from happening, behavior must be positively added. (consensus about future build/runtime expectations must occur)
